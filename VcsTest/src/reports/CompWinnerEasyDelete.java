...
/**
 * Servlet implementation class for Servlet: CompWinnerEasyDelete
 *
 */
 public class CompWinnerEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public CompWinnerEasyDelete() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Winner Of Competition Organized In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Competition Winner")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				//kout.println("<br><br><h1>Winner Of Competition Organized In VCS</h1>");

				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from comp_winner");
				int c=1;
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				kout.println("<form action='/VcsTest/CompWinnerEasyHelp' method='post'>");
				while(rs.next())
				{
					...
					kout.println("<tr><td><input type='submit' value='Delete'><td>Sr No:<td>Result Date<td>First<td>Second<td>Third<td>Total Participants<td>");
					
					kout.println("<tr><td><input type='checkbox' name='"+"c"+c+"' value='"+str+"'>");
					
					kout.println("<td>"+c);
					kout.println("<td>"+rs.getString("rdate"));
					kout.println("<td>"+rs.getString("firstp"));
					kout.println("<td>"+rs.getString("secondp"));
					kout.println("<td>"+rs.getString("thirdp"));
					kout.println("<td>"+rs.getString("nop"));
					c=c+1;
				...
				if(c==1)
					kout.println("<br><br><br><h1>Sorry No Competition Winner Is Declared Till Now");
			}
			catch(Exception k)
			{
