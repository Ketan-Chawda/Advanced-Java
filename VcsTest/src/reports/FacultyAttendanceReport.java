...
/**
 * Servlet implementation class for Servlet: FacultyAttendanceReport
 *
 */
 public class FacultyAttendanceReport extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public FacultyAttendanceReport() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Faculty's Attendance Report In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=150><blockquote><h2>"+Styles.ccase("Report Title : "+"Faculty's Attendance")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				//kout.println("<br><br><h1>Faculty's Attendance Report In VCS</h1>");

				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select distinct fid from faculty_attend");
				int c=1;
				...
				while(rs.next())
				{
					...
					st=mycon.createStatement();
					ResultSet rs1=st.executeQuery("select distinct ondate,status from faculty_attend where fid='"+str+"'");
					while(rs1.next())
					{
					kout.println("<tr><td>Sr No:<td>Date<td>Status");//<td>Time");
					kout.println("<tr><td>"+c);
					
					kout.println("<td>"+rs1.getString("ondate"));
					kout.println("<td>"+rs1.getString("status"));
					//kout.println("<td>"+rs1.getString("ontime"));
