...
/**
 * Servlet implementation class for Servlet: MgtFinalAttendEasyDelete
 *
 */
 public class MgtFinalAttendEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public MgtFinalAttendEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Management's Final Attendance In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Management's Final Attendance")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				
				//kout.println("<br><br><h1>Management's Final Attendance Report In VCS</h1>");
				st=mycon.createStatement();
				//response.sendRedirect("/VcsTest/ServletMgtAttendCount");
				ResultSet rs=st.executeQuery("select * from mgt_cattend");

				int c=1;
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				kout.println("<form action='/VcsTest/MgtFinalAttendEasyHelp' method='post'>");
				while(rs.next())
				{
				...
				if(c==1)
				kout.println("<br><br><br><h1>Sorry No Management Is At Present Involved With VCS");
			}
			catch(Exception k)
			{
