...
/**
 * Servlet implementation class for Servlet: FacultyAttendanceEasyDelete
 *
 */
 public class FacultyAttendanceEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public FacultyAttendanceEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Faculty's Attendance In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Faculty Attendance")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				
				//kout.println("<br><br><h1>Faculty's Attendance Report In VCS</h1>");

				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from faculty_attend");
				int c=1;
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				kout.println("<form action='/VcsTest/FacultyAttendanceEasyHelp' method='post'>");
				while(rs.next())
				{
				...
				if(c==1)
				kout.println("<br><br><br><h1>Sorry No Faculty Is Still Present Till Now");
			}
			catch(Exception k)
			{
