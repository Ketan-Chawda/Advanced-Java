...
/**
 * Servlet implementation class for Servlet: MgtAttendanceEasyDelete
 *
 */
 public class MgtAttendanceEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public MgtAttendanceEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Management's Attendance Report In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Management's Attendance")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				
				//kout.println("<br><br><h1>Management's Attendance Report In VCS</h1>");
	
				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from mgt_attend");
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				int c=1;
				kout.println("<form action='/VcsTest/MgtAttendanceEasyHelp' method='post'>");
				while(rs.next())
				{
					...
					kout.println("<tr><td><input type='submit' value='Delete'><td>Sr No:<td>Date<td>Status<td>");
					kout.println("<tr><td><input type='checkbox' name='"+"c"+c+"' value='"+str+"'>");
					
					kout.println("<td>"+c);
					kout.println("<td>"+rs.getString("ondate"));
					kout.println("<td>"+rs.getString("status"));
					c=c+1;
				...
				if(c==1)
				kout.println("<br><br><br><h1>Sorry No Management Is Still Present Till Now");
			}
			catch(Exception k)
			{
