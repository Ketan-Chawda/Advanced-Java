...
/**
 * Servlet implementation class for Servlet: MgtBlogsEasyDelete
 *
 */
 public class MgtBlogsEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public MgtBlogsEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Blogs Of Management In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Management's Blogs")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				
				//kout.println("<br><br><h1>View Blogs Of Management In VCS</h1>");

				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from blog_master where coursename='optional' and userid in (select userid from login_master where role='management')");
				int c=1;
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				kout.println("<form action='/VcsTest/MgtBlogsEasyHelp' method='post'>");
				while(rs.next())
				{
					...
					kout.println("<tr><td>Sr No:<td>"+c+"<tr><td>Given By:<td>"+rs.getString("userid")+"<tr><td>Blog Date & Time:<td>"+rs.getString("bdate"));
					kout.println("<tr><td colspan=100%><center>Blog Message:</center>");
					kout.println("<tr><td colspan=100%>"+rs.getString("bmsg"));
				...
				st.close();
				if(c==1)
					kout.println("<br><br><h1>Sorry No Blog Have Been Created Or Available Of Management At Present");
				}
			catch(Exception k)
			{
