...
/**
 * Servlet implementation class for Servlet: StudentBlogsEasyDelete
 *
 */
 public class StudentBlogsEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public StudentBlogsEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Blogs Of Students In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Student's Blogs")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
		
				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from blog_master where not coursename='optional' ");
				int c=1;
				kout.println("<form action='/VcsTest/StudentBlogsEasyHelp' method='post'>");
				while(rs.next())
				{
				...
				if(c==1)
					kout.println("<br><br><h1>Sorry No Blog Have Been Created Or Available By Student At Present");
				}
			catch(Exception k)
			{
