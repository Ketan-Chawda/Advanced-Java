...
/**
 * Servlet implementation class for Servlet: StudentFinalAttendEasyDelete
 *
 */
 public class StudentFinalAttendEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public StudentFinalAttendEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Student's Final Attendance In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Student's Final Attendance")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
		
				//kout.println("<br><br><h1>Student's Final Attendance Report In VCS</h1>");
	
				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select * from student_cattend");
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				int c=1;
				kout.println("<form action='/VcsTest/StudentFinalAttendEasyHelp' method='post'>");
				while(rs.next())
				{
					...
					kout.println("<tr><td><input type='submit' value='Delete'><td>Sr No:<td>Course-Name<td>Total<td>Lastest Report Date<td>");
					
					kout.println("<tr><td><input type='checkbox' name='"+"c"+c+"' value='"+str+"'>");
					
					kout.println("<td>"+c);
					kout.println("<td>"+rs.getString("coursename"));
					kout.println("<td>"+rs.getString("total"));
					kout.println("<td>"+rs.getString("udate"));
					c=c+1;
				...
				if(c==1)
				kout.println("<br><br><br><h1>Sorry No Student Is Currently Studying In VCS");
				kout.println("<tr><td colspan=100%><center>"+Fortable.display(33,"<<->>"));
