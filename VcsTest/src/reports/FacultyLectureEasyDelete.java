...
/**
 * Servlet implementation class for Servlet: FacultyLectureEasyDelete
 *
 */
 public class FacultyLectureEasyDelete extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
   
    /* (non-Java-doc)
	 * @see javax.servlet.http.HttpServlet#HttpServlet()
	 */
	public FacultyLectureEasyDelete() {
		super();
	}   	
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
	{
		...
		kout.println("<html><head><title>Easy Deletion Of Faculty Lecture's In VCS</title></head>");
		...
		try
		{
			...
			try
			{
				...
				kout.println("<tr><td colspan=75%><blockquote><h2>"+Styles.ccase("Operation On : "+" Lecture")+"<td><h2><a href='javascript:history.go(-1)'>Back</a>");
				kout.println("</table>");
				kout.println("<br><br><br>**************************************************************************************************************</center><br>");
				//kout.println("<br><br><h1>View Faculty Lecture's In VCS</h1>");

				st1=mycon.createStatement();
				st=mycon.createStatement();
				ResultSet rs=st.executeQuery("select distinct tofid from subject_master");

				int c=1,k=1;
				kout.println("<center><table align='right' width=95% border=2 bordercolor=black>");
				kout.println("<form action='/VcsTest/FacultyLectureEasyHelp' method='post'>");
				while(rs.next())
				{
					String str=rs.getString("tofid");
					...
					ResultSet rs1=st1.executeQuery("select * from subject_master where tofid='"+str+"'");
					kout.println("<tr><td colspan=95%>Sr No:"+c);
					k=1;
					while(rs1.next())
					{
					kout.println("<tr><td><input type='submit' value='Delete'><td>Total:<td>CourseName<td>Subject<td>Subdesc<td>");
					
					kout.println("<tr><td><input type='checkbox' name='"+"c"+k+"' value='"+str+"'>");
					
					kout.println("<td>"+k);
					kout.println("<td>"+rs1.getString("coursename"));
					kout.println("<td>"+rs1.getString("subject"));
					kout.println("<td>"+rs1.getString("subdesc"));
				...
				if(c==1)
				kout.println("<br><br><br><h1>Sorry No NewsLetter Is Declared Or Defined Till Now");
				}
			catch(Exception k)
			{
