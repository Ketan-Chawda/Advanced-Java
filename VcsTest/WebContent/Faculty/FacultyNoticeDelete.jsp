...
<%
        ...
        out.println("document.getElementById('nid').options[z]=opt;");
        out.println("document.getElementById('nid').options[z].text=item;");
        out.println("document.getElementById('nid').options[z].value=item;");
        out.println("z++;}");
...
<%
        try
		{
			...
			ResultSet rs=stt.executeQuery("select nid from notice where nprior>3 or noticefrom='"+trole+"' ");	
			while(rs.next())
			{
				data=rs.getString("nid");
...
<ul>
	...
	<li class="current"><a href="FacultyNotice.jsp" class="m5">NOTICE</a></li>
...
<ul class="categories">
	...
	<li><span><a href="FacultyNoticeCreate.jsp">Create
	Notice</a></span></li>
	<li><span><a href="FacultyNoticeDelete.jsp">Delete
	Notice</a></span></li>
...
<h4>Notice Deletion</h4>
</font></legend>
<dd><br>
<form name="recover" id="recover" method="post"
	action="/VcsTest/ServletFacultyNoticeDelete">
<table name="frm_frm" id="frm_frm" align="center" width="60%">
	...
	<tr>
		...
		<td colspan="3"><select name="nid" id="nid"
